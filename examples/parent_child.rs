...
fn interaction_panel(mut contexts: EguiContexts, mut parent_event_writer: EventWriter<CudEvent<Building, String>>, mut child_event_writer: EventWriter<CudEvent<Level, String>>) {
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Create or modify child").clicked() {
                    child_event_writer.send(CudEvent::create_or_modify_child("Building".into(), "Level".into(), Level));
                }
                if ui.button("Modify child").clicked() {
                    child_event_writer.send(CudEvent::update_child("Building".into(), "Level".into(), Level));
                }
                if ui.button("Remove child").clicked() {
                    child_event_writer.send(CudEvent::delete_child("Building".into(), "Level".into(), Level));
                }
            });
